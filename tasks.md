...
- task-02 | Gemini returns invalid JSON | status: done — `max_output_tokens=32768` (the default cap truncated long trips mid-object) + one retry of the call for truncated/empty/field-less responses. Self-check: `gmap_planner/test_gemini_extract.py`. `8ea63c0`
...
- task-05 | macOS app has no Playwright browser | status: done — `build_app.sh` installs Chromium with `PLAYWRIGHT_BROWSERS_PATH=0` so it is bundled into the `.app`, restores the exec bits PyInstaller drops, and the runtime points `PLAYWRIGHT_BROWSERS_PATH` at it; `ensure_chromium()` downloads via Playwright's node driver when a build has none. No Chrome install needed. `df1aa73`
