...
- task-04 | macOS (Apple Silicon) bugs | status: done — `--collect-all webview` (pywebview's JS assets), updater picks the `.dmg` by CPU and offers an Intel Mac nothing rather than an arm64 build, deep-codesign verify downgraded to a warning, stale `login.bat` / `TripMapMaker.app` references corrected, and CI now runs the frozen app's `--selfcheck` on a real macOS runner plus a launch test of the bundled browser. Releases are Apple Silicon only (the Intel cross-build was dropped: it shipped an arm64 browser inside the Intel app). `2b0760d`, `a0c84eb`
