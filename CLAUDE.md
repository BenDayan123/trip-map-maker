...
- **`analytics.py`** — appends each publish to a Google Sheet (`record_publish` → one row: `Created At, Trip Name, Maps, Map Links`, written `USER_ENTERED` so the timestamp is a real date; `fetch_rows` reads columns `A:D` back and normalizes headers to canonical keys — `Created At` → `created_at` — so an un-migrated snake_case sheet still works). `ensure_layout` makes the Sheet readable in the browser: display header, frozen/bold header row, banding, column widths, basic filter, and a **live formula summary box** in `F:G` (totals + maps this calendar month); it's idempotent (one read when already tidy), migrates an old 3-column sheet after duplicating it to a `Backup (pre-format)` tab, and `_worksheet` picks the first tab that *isn't* that backup. Auth reuses the `GCP_SA_JSON` service account via `gspread`; target Sheet is `ANALYTICS_SHEET_ID`. Best-effort — any Sheets failure logs a warning and returns, never breaking map generation. Read by the **`pages/Analytics.py`** page (open, no password gate; has a "Tidy up the Sheet" button).
